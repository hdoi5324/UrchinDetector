    ...
    def forward(
        ...
        if torch.rand(1) < self.p:
            ...
            if target is not None:
                ...
                if "masks" in target:
                    raise "Not implemented: vertical flip on masks not implemented"
                if "keypoints" in target:
                    raise "Not implemented: vertical flip on keypoints not implemented"
...
class RandomShortestSize(nn.Module):
    ...
    def forward(
        ...
        if target is not None:
            bbox = target["boxes"]
            bbox = resize_boxes(bbox, (orig_height, orig_width), (new_height, new_width))
            target["boxes"] = bbox
            if "masks" in target:
                target["masks"] = F.resize(
                    target["masks"], [new_height, new_width], interpolation=InterpolationMode.NEAREST
        ...
        #print(f"Was {orig_height}, {orig_width}, now {new_height}, {new_width}")
        return image, target
